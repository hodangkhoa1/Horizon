...
import com.team.horizon.models.Constants;
...
public class FunctionSendEmail {
    ...
    public boolean sendMailChangePassword() {
        try {
            ...
            email.setAuthenticator(new DefaultAuthenticator(Encrypt.decrypt(Constants.MY_EMAIL, SECRET_KEY), Encrypt.decrypt(Constants.MY_PASSWORD, SECRET_KEY)));
            email.setSSLOnConnect(true);
            email.setFrom(Encrypt.decrypt(Constants.MY_EMAIL, SECRET_KEY));
            ...
            email.setHtmlMsg("<html><section class=\"confirm-email\">\n"
                    ...
                    + "                    <p style=\"padding-top: 30px; font-size: 18px;\">Dear " + account.getUserName()+ ",</p>\n"
                    + "                    <p style=\"padding-top: 30px; font-size: 18px;\">Your reset code: " + codeVerify + "</p>\n"
                    + "                    <p style=\"padding-top: 30px; font-size: 18px;\">Thank you for trusting Horizon.</p>\n"
                    + "                </div>\n"
                    + "            </div>\n"
                    + "    </section></html>");
    ...
    public boolean sendMailVerifyAccount() {
        try {
            ...
            email.setHtmlMsg("<html><section class=\"confirm-email\">\n"
                    ...
                    + "            <div class=\"confirm-content\" style=\"margin-bottom: 2rem; padding-left: 1rem;\">\n"
                    + "                <div class=\"content\" style=\"margin-bottom: 3rem;\">\n"
                    + "                    <h1 style=\"padding-top: 30px;\">Verify account email <span style=\"color: rgb(113, 23, 234);\">Horizon</span></h1>\n"
                    + "                    <p style=\"padding-top: 30px; font-size: 18px;\">Dear " + account.getUserName() + ",</p>\n"
                    + "                    <p style=\"padding-top: 30px; font-size: 18px;\">Thank you for creating a Horizon account.</p>\n"
                    + "                    <p style=\"font-size: 18px;\">Please click the [Confirm] button below to complete the registration process.</p>\n"
                    + "                    <p style=\"font-size: 18px;\">Failure to confirm your e-mail account within 48 hours will result in account deletion. If so, you will have to start the membership registration process again and receive a new confirmation e-mail.</p>\n"
                    + "                </div>\n"
                    + "\n"
                    + "                <div class=\"confirm-btn\" style=\"display: flex;\n"
                    + "                justify-content: center;\n"
                    + "                align-items: center;\">\n"
                    + "                    <a href=\"http://localhost:8080/Horizon/verify?uid=" + Encrypt.encrypt(account.getEmail(), SECRET_KEY) + "\" class=\"btn-reset\" style=\"background-color: #8412c0;\n"
                    + "                    color: white;\n"
                    + "                    border: none;\n"
                    + "                    border-radius: 2px;\n"
                    + "                    padding: 14px;\n"
